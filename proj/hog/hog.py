...
def make_averaged(original_function, total_samples=1000):
    ...
    def average(*args):
        total_value = 0
        remain_samples = total_samples
        while remain_samples > 0:
            total_value += original_function(*args)
            remain_samples -= 1
        return total_value / total_samples
    return average
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while cur_num <= 10:
        ...
        while samples:
            cur_score += roll_dice(cur_num, dice)
            samples -= 1
        if cur_score > max_score:
            max_score = cur_score
            max_scoring_num = cur_num
        cur_num += 1
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    point = tail_points(opponent_score)
    if point >= threshold:
        return 0
    else:
        return num_rolls
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    point = square_update(0, score, opponent_score) - score
    if point >= threshold:
        return 0
    else:
        return num_rolls
...
def final_strategy(score, opponent_score, threshold=20):
    """Write a brief description of your final strategy.

    This strategy try to combine tail strategy and square_strategy.
    """
    # BEGIN PROBLEM 12
    point = tail_points(opponent_score)
    if point >= threshold:
